 ...
 */
public class SKKUOperationServer implements InitializingBean {
	private static final Logger s_logger = LoggerFactory.getLogger(SKKUOperationServer.class);
...
//    		String sessionId = Integer.toHexString(sim.hashCode());
    ...
    private StartableExecution<List<String>> startSimulation(Submodel submodel) {
		...
		if ( req.getSimulationTimeout() != null ) {
			var timed = AsyncExecutions.timed(simulation, req.getSimulationTimeout());
			timed.start();
		}
		else {
			simulation.start();
